 ...
 public:
  ...
  class Edge {
   public:
    ...
    Node node2() const {
      ...
      //return Node(graph_,std::get<1> (graph_->edges_[index_]));
    }
    ...
    bool operator==(const Edge& e) const {
      return (graph_==e.graph_ && ((index1_ ==e.index1_ && index2_ == e.index2_)||
    ...
    bool operator<(const Edge& e) const {
      if (graph_<e.graph_){
        return true;}
      else if(graph_==e.graph_ && std::min(index1_,index2_)<std::min(e.index1_,e.index2_)){
        return true;}
      else if(graph_==e.graph_ && std::min(index1_, index2_)==std::min(e.index1_,e.index2_)
             && std::max(index1_,index2_)<std::max(e.index1_,e.index2_)){
        return true;}
      return false;
    }
   ...
   private:
    ...
    Edge(const graph_type* graph, size_type index1, size_type index2)
      : graph_(const_cast<graph_type*>(graph)), index1_(index1), index2_(index2){
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (size_type i=0; i<num_edges(); ++i){
      if ((edges_[i].first==a.index_ && edges_[i].second==b.index_) ||
          (edges_[i].first==b.index_ && edges_[i].second==a.index_)){
        return true;}
    }
    return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
   if (!has_edge(a,b)){
     edges_.push_back(std::make_pair(a.index_, b.index_));
   }
