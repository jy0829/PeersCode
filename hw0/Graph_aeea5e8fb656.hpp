...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE HERE
    internal_nodes.push_back(position); // Adding nodes to the internal_nodes vector
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
    // Zero vector equal to the number of nodes.
    if (internal_edges.size()==0)
      adjacency.assign(num_nodes(),std::vector<std::pair<size_type, size_type>> (0));
