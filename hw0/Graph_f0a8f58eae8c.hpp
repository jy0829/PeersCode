...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    bool operator==(const Node& n) const {
      // HW0: YOUR CODE HERE
      if (n.index() == index_ && n.graph_ == graph_){
        return true;
      }
      return false;
    }
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    if (n.graph_ == this && n.index() < num_nodes()){
      return true;
    }
    return false;
  }
  ...
  class Edge {
   public:
    ...
    bool operator==(const Edge& e) const {
      if (std::min(id_a_,id_b_) == std::min(e.id_a_, e.id_b_) &&
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    edge_type e_aux;
    for(std::pair<size_type, size_type> p: edges_){
      e_aux = Edge(this, p.first, p.second);
      if(e == e_aux){
        return true;
      }
    }
