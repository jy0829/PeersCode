...
class Graph {
 private:
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      return (this->g)->node_list[this->node_id];
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HERE
      return (this->g)->edge_list[this->edge_id].node1;      // Invalid Node
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return (this->g)->edge_list[this->edge_id].node2; 
    }
   ...
   private:
    ...
    Edge(const Graph* graph, size_type uid)
      : g(const_cast<Graph*> (graph)), edge_id(uid) {
      } 
  };
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // HW0: YOUR CODE HERE
    if (a.index() < node_adj_list.size() && b.index() < node_adj_list.size()){
      if (!node_adj_list[a.index()].empty()){
	for (auto& edge_uid : node_adj_list[a.index()]){
	  if (edge(edge_uid).node1() == b || edge(edge_uid).node2() == b){
	    return true;
	  }
        }
      }
  ...
  Edge add_edge(const Node& a, const Node& b) {
	...
	for (auto& edge_uid : node_adj_list[a.index()]){
	  if (edge(edge_uid).node1() == b || edge(edge_uid).node2() == b){
	    return edge(edge_uid);
	  }
        }
      }
    } 
    edge_data new_edge = {a, b};
 ...
 private:
  ...
  void insert_into_adj_list(const Node& a, const Node& b, size_type uid){
    ...
    node_adj_list[a.index()].push_back(uid);
    node_adj_list[b.index()].push_back(uid);
  } 
