...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    bool operator==(const Node& n) const {
      return ((this->uid_ == n.uid_) and (this->graph_ == n.graph_));
    }
  ...
  Edge edge(size_type i) const {
    std::pair<size_type,size_type> ed = this->v_edges[i];
    return Edge(this,std::get<0>(ed),std::get<1>(ed));
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    std::pair<size_type,size_type> ed = {std::min(a.uid_,b.uid_),
        std::max(a.uid_,b.uid_)};
    if (this->edges.find(ed) != this->edges.end()) {
      return true;
    } else {
      return false;
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    if (!has_edge(a,b)) {
      std::pair<size_type,size_type> ed = {std::min(a.uid_,b.uid_),
          std::max(a.uid_,b.uid_)}; 
      this->edges.insert(ed);
      this->v_edges.push_back(ed);
    }
 ...
 private:
  ...
  std::set<std::pair<size_type,size_type>> edges; 
  std::vector<std::pair<size_type,size_type>> v_edges;
};
