...
class Graph {
 private:
  ...
  //std::map<size_type, Node&> index_to_node;
  //std::vector<void*> index_to_node;
 ...
 public:
  ...
  bool has_node(const Node& n) const {
    ...
    return node_positions.size() > n.index_;
  }
  ...
  Node node(size_type i) const {
    ...
    return index_to_node[i];        // Invalid node
  }
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HERE
      return graph_->node(graph_->node1_index[index_]);
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return graph_->node(graph_->node2_index[index_]);
    }
  ...
  Edge edge(size_type i) const {
    // HW0: YOUR CODE HERE
    return index_to_edge[i];        // Invalid Edge
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (size_type i = 0; i < num_edges_; i++) {
      if (node1_index[i] == a.index() and node2_index[i] == b.index()) {
        return True;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    Edge e(this, node1_index.size() - 1);
