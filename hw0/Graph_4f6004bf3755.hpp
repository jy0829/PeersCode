...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    sp_adj_matrix.push_back(vec);
    return Node(this, points.size()-1);        // Invalid node
  }
  ...
  class Edge {
   ...
   private:
    ...
    size_type index1_;
    size_type index2_;
    Edge(Node n1, Node n2): index1_(n1.index()), index2_(n2.index()) {}
  };
  ...
  size_type num_edges() const {
    // HW0: YOUR CODE HERE
    size_type nedges = 0;
    for(uint i = 0; i < sp_adj_matrix.size(); ++i) {
      nedges += sp_adj_matrix[i].size();
    } 
    return nedges/2;
  }
  ...
  Edge edge(size_type i) const {
    ...
    for(uint j = 0; j < num_nodes(); ++j) {
      for(uint k = 0; k < sp_adj_matrix[j].size(); ++k) {
        if(sp_adj_matrix[j][k] < j) {
          if(nedges == i) return Edge(node(j), node(sp_adj_matrix[j][k]));
          nedges += 1;
        }
      }
    }
