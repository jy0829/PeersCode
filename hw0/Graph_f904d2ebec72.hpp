...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    // add the new internal_nodes to the vector of nodes
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    (void) n;            // Quiet compiler warning
    if(n.graph_ == this && n.index_ < size())
        return true;
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for(std::vector<internal_edge>::const_iterator it = edges.begin();
            it != edges.end(); it++){
        if(((*it).node1 == a && (*it).node2 == b) ||
                ((*it).node1 == b && (*it).node2 == a))
            return true;
    }
    return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    assert(has_node(b));
    if(has_edge(a,b)){
        for(std::vector<internal_edge>::iterator it = edges.begin();
                it != edges.end(); it++){
            if(((*it).node1 == a && (*it).node2 == b) ||
                    ((*it).node1 == b && (*it).node2 == a))
                return Edge(this, (*it).index);
        }
    } else {
        size_type old_num_edge = num_edges();
        internal_edge new_edge;
        new_edge.index = old_num_edge;
        new_edge.node1 = a;
        new_edge.node2 = b;
        edges.push_back(new_edge);
        return Edge(this, old_num_edge);
    }
