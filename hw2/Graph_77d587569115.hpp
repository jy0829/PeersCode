...
//std::vector<E> Value_edge_vect;
 ...
 using edge_value_type = E;
  struct node_elt {
    ...
    bool operator==(const size_type& n) const { return node_==n; }
  };

std::vector<node_elt> Point_vect;
std::vector<size_type> index_of_node;
std::vector<std::vector<adj_elt>> Adj_vect;
  ...
  class Node : private totally_ordered<Node>  {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      //return Point();
      return pgraph->Point_vect[dx].point;
    }
    /**Return the modifiable position of the node*/
    Point& position(){
      return pgraph->Point_vect[dx].point;
    }
    ...
    size_type index() const {
      // HW0: YOUR CODE HERE
      //return size_type(-1);
      return pgraph->Point_vect[dx].dx_node;
    }
    ...
    node_value_type& value(){
      return pgraph->Point_vect[dx].value;
...
/**edge_begin() return an the begining iterator  that will go through all the edges connected to the node*/
  ...
  size_type size() const {
    // HW0: YOUR CODE HERE
    //return 0;
    return index_of_node.size(); /**here dont need this->Point_vect.size() because there is 
    no ambiguity */
  ...
  Node add_node(const Point& position, const node_value_type& myval = node_value_type()) {
    ...
    // return Node(this, index_of_node.size()-1);
    node_elt mynode;
    mynode.value = myval;
    mynode.point = position;
    mynode.dx_node =size();
    index_of_node.push_back(Point_vect.size());
    Point_vect.push_back(mynode);
    std::vector<adj_elt> adj;
    Adj_vect.push_back(adj);
    return Node(this, index_of_node[size()-1]);  }
  ...
  bool has_node(const Node& n) const {
    ...
    //(void) n;            // Quiet compiler warning
    //return false;
    return (n.pgraph == this) && (n.index() < size());
  }
  ...
  Node node(size_type i) const {
    ...
    //return Node();        // Invalid node
    return Node(this,index_of_node[i]);
  }
  //------------------------------------------------------------------------------------------------------------------------------
  /**Remove node @a node as well as its incident edges
  *@param[in] node: node to remove
  *@returns 1 if @a node is not in the graph, 0 otherwise. (could be a boolean function)
  *@post num_nodes= num_nodes-1, num_edges=numedges-degree of @a node
  *@Complexity O(node.degree()*sum) with sum being equal to the sum of degrees of adj nodes
...
size_type remove_node(const Node& node){
  ...
  int bound =Adj_vect[node.dx].size();
  for (int i = 0;i<bound;i++){
    size_type id = Adj_vect[node.dx][i].node_;
    int bound2 =Adj_vect[id].size();
    for(int j=0;i<bound2;j++){
      if(Adj_vect[id][j].node_==node.dx){//select the one to remove out of the list
      Adj_vect[id][j]=Adj_vect[id].back();
      Adj_vect[id].pop_back();
      break;//break when the node is removed 
    }
    }
  }
Adj_vect[node.dx].clear();
index_of_node[Point_vect[node.dx].dx_node]=index_of_node.back();//remove incident edges
Point_vect[index_of_node[Point_vect[node.dx].dx_node]].dx_node=Point_vect[node.dx].dx_node;
index_of_node.pop_back();
return 1;
}
...
//in add_edge we will have to be careful about sorting indices
    ...
    edge_value_type& value(){
      auto& var_adj = pgraph->Adj_vect[nmin()];
      auto iter = std::find(var_adj.begin(), var_adj.end(), nmax());
      //assert(iter !=var_adj.end());//safe
      return (*iter).value;
    }
    /**Return value of edge const form*/
   ...
   private:
    ...
    Edge(const Graph* _pgraph, size_type index_1, size_type index_2)
        : dx1(index_1), dx2(index_2), pgraph(const_cast<Graph*>(_pgraph)){} //pointer has not to be changed here: it is constant
    size_type dx1;
    size_type dx2; // the node indices in Graph
     Graph* pgraph; 

    size_type nmin() const{
  return std::min(dx1,dx2);

}
size_type nmax() const {
  return std::max(dx1,dx2);
}
  };
  ...
  //   // HW0: YOUR CODE HERE
  //   size_type double_num_edges;
  //   double_num_edges=0;
  //   //int bound = index_of_node.size();
  //   for(int i=0;i<index_of_node.size();i++){
  //     double_num_edges+=Adj_vect[index_of_node[i]].size();
  //   }
  //   //size_type num_edges=double_num_edges/2;
  //   //return num_edges;
  //   return double_num_edges/2;;
  // }

  size_type num_edges() const {
    size_type numb_edges=0;
    for(size_type i=0;i<Adj_vect.size();++i)
      for(size_type j=0;j<Adj_vect[i].size();++j)
        if(Point_vect[i].dx_node < Point_vect[Adj_vect[i][j].node_].dx_node)
          ++numb_edges;
    return numb_edges;}
...
size_type id1=a.index();
size_type id2=b.index();
for(size_type i=0;i<Adj_vect[index_of_node[id1]].size();++i)
  if(Point_vect[Adj_vect[index_of_node[id1]][i].node_].dx_node==id2)
    return true;
  return false;
  ...
  Edge add_edge(const Node& a, const Node& b,const edge_value_type& eval = edge_value_type()) {
   ...
   //  Edge res_edge = Edge(this, a.index(), b.index());
   //  res_edge.value()=eval;
     adj_elt a_;adj_elt b_;
     a_.node_=a.dx;
     b_.node_=b.dx;
     Adj_vect[a_.node_].push_back(b_);
     Adj_vect[b_.node_].push_back(a_);
}
Edge myedge = Edge(this,a.dx,b.dx);
myedge.value() = eval;
  ...
  /**remove_edge is removing an edge defined by two nodes
  *returns 0 if edge is in the graph else, returns 1
  *@param[in] @a node1 and @a node2, the nodes that define the edge to remove
  *@post num_edges=num_edges-1
  * Complexity O(sum of degrees of node1 and node2)
  */

  size_type remove_edge(const Node& node1, const Node& node2){
        if(!(has_edge(node1, node2))){
          return 0;}
        for (int i =0; i <Adj_vect[node2.dx].size(); i++) {
            if (Adj_vect[node2.dx][i].node_ ==node1.dx) {
                     Adj_vect[node2.dx][i] = Adj_vect[node2.dx].back();
                     Adj_vect[node2.dx].pop_back();
                     break;
      }
    }
        for (int j = 0; j < Adj_vect[node1.dx].size(); j++) {
           if (Adj_vect[node1.dx][j].node_ == node2.dx) {
        Adj_vect[node1.dx][j] = Adj_vect[node1.dx].back();
        Adj_vect[node1.dx].pop_back();
        break;
      }
    }
    return 1;
  }
  /**remve_edge(const Edge&) remove an edge passed in argument
  *@param[in] @a edge: the edge to remove
  *@post num_edges=num_edges-1
  *Complexity: Complexity of size_type remove_edge(const Node& , const Node&)
  */
  ...
  /**edge_iterator remove_edge(edge_iterator ) removes the edge at the location given by 
  the iterator
  *@param[in] iter: the edge iterator that point the edge to remove
  *@returns the next edge iterator of the adjacency list
  *@post num_edges=num_edges-1
  *Complexity: Complexity of size_type remove_edge(const Node& , const Node&)
  */
edge_iterator remove_edge(edge_iterator iter){
  remove_edge((*iter));
  iter.aux();
  return iter;
}
  ...
  class NodeIterator:private totally_ordered<NodeIterator> {
    ...
    */
    Node operator*() const{
      return Node(pgraph,pgraph->index_of_node[id]);
    }
    ...
    */
    bool operator==(const NodeIterator& iter_) const{
      return (pgraph == iter_.pgraph) && (id==iter_.id);
    }

   private:
    friend class Graph;
    // HW1 #2: YOUR CODE HERE
    NodeIterator(const Graph* _pgraph, size_type _id):
    pgraph(const_cast<Graph*>(_pgraph)),id(_id){
    }
    const Graph* pgraph;
    size_type id;
  };
  ...
  */
   ...
   /**node_iterator remove_node(node_iterator) removes node that is pointed by the iterator
  *@param[in] iter: the node iterator that points to the node to remove
  *@ returns the iterator pointing to the follozing nodes zhich new id is the old id of the node that 
  was removed.
  *@ post num_nodes = num_nodes-1 and num_degree=num_degree- *iter.degree()
  *@Complexity O(node.degree()*sum) with sum being equal to the sum of degrees of adj nodes
   */
   node_iterator remove_node(node_iterator iter){
    remove_node(*iter);
    return iter;
   }
  ...
  class IncidentIterator: private equality_comparable<IncidentIterator> {
    ...
    */
    Edge operator*() const{
      return Edge(_pgraph,_dx,_pgraph->Adj_vect[_dx][rank_in_Adj].node_);
    }
  ...
  class EdgeIterator:private equality_comparable<EdgeIterator> {
    ...
    */
     Edge operator*() const{
      return Edge(_pgraph,nd1,_pgraph->Adj_vect[nd1][nd2].node_);
     }
    /** operator++() is incrementing the iterator which points to the list of adjacency of the specific node
    *@pre iterator should not be equal to the end iterator
    *@return the iterator incremented by 1
    ...
    */
    bool operator==(const EdgeIterator& iter) const{
    	return (_pgraph==iter._pgraph) && (nd1==iter.nd1) && (nd2==iter.nd2);
    }
   ...
   private:
    ...
    size_type nd1;
    size_type nd2;
    EdgeIterator(const Graph* pgraph_,size_type _nd1,size_type _nd2):
    _pgraph(const_cast<Graph*>(pgraph_)),
    nd1(_nd1),nd2(_nd2){aux();}
    ...
    */
      void aux(){
        while(nd1<_pgraph->Adj_vect.size()){
          while (nd2<_pgraph->Adj_vect[nd1].size()){
            if(nd1<_pgraph->Adj_vect[nd1][nd2].node_) 
              {return;}
            ++nd2;
          }
          ++nd1;
