...
Point spring_force(Node n) {
  ...
  for(; first != last; ++first) {
    auto edge = *first;
    auto node_1 = edge.node2();
    if(edge.node1() != n) {
      node_1 = edge.node1();
    }
    Point p = n.position();
    Point p_incident = node_1.position();
    double distance = edge.Length();
    result += -K*(p - p_incident)/distance*(distance - L);
  }
  return result;
}
...
Point gravity_force(Node n) {
  return -1*n.value().mass*Point(0, 0, grav);
}
...
struct Problem1Force {
  ...
  Point operator()(NODE n, double t) {
    ...
    Point spring_result = spring_force(n);
    Point gravity_result = gravity_force(n);
    return spring_result + gravity_result;
  }
};
...
int main(int argc, char** argv)
{
  ...
  double mass_initial = 1.0/N;
  Point vel_initial = Point(0, 0, 0);
  auto first = graph.node_begin();
  auto last = graph.node_end();
  for(; first != last; ++first) {
    auto node = *first;
    node.value().mass = mass_initial;
    node.value().vel = vel_initial;
  }
