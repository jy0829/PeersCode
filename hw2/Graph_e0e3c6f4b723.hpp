...
class Graph {
  ...
  */
  ...
  Node add_node(const Point& position, const node_value_type& val = node_value_type()) {
    ...
    std::vector<std::pair<size_type, edge_value_type>> empty_list;
  ...
  /** A function that removes a node and its associated edges from the graph.
   * @param n     node to be removed
   * @return      returns size_type value:
   *                1 if @a n and associated edges were successfully removed
   *                0 if @a n does not exist in graph
   *
   * @pre         a valid Graph::Node object is passed in
   * @post        @a n node and all edges connected to @a n are removed
   * @post        all @a i > n.index() are decremented by 1 in @a nodes
   *              and @a adj_list, where 0 <= @a i <= num_nodes()-1
   * @post        the size of @a nodes and @a adj_list are decremented by 1
   * @post        numEdges decremented by degree(n)
   *
   * Complexity: O(num_nodes), assuming a sparse graph (as mentioned by
   *              professor on canvas).
   */
  size_type remove_node(const Node& n){
    ...
    std::vector<std::pair<size_type, edge_value_type>> x = adj_list[n.index()];
    for(size_type i = 0; i < x.size(); ++i){
      remove_edge(Edge(this, nid, x[i].first));
    }

    //decrement all values after node removed in adjacency list -> but this complexity is O(2m)
    for(size_type i = 0; i < num_nodes(); ++i){
      for(size_type j = 0; j < adj_list[i].size(); ++j){
        if(adj_list[i][j].first > nid){
          --adj_list[i][j].first;
        }
      }
    }

    //delete node in node data structure
    nodes.erase(nodes.begin()+nid);

    //delete node in adjacency list
    adj_list.erase(adj_list.begin()+nid);

    return 1;
  }

 /* A node at the location pointed at by a particular node_iterator and all of
  * the node's associated edges are deleted from the graph.
  * @param n_it   a valid node iterator pointing to the node to be deleted
  * @return       a node_iterator pointing to the next node after the node
  *               which was deleted.
  *
  * This function calls size_type remove_node(const Node& n). Thus, the @pre
  * & @post conditions and the complexity are the same as those listed for
  * size_type remove_node(const Node& n).
  *
  */
  ...
  class Edge : private totally_ordered<Edge>{
   public:
    ...
    edge_value_type& value(){
      // will only use one edge to store the updated 'value' (n1 < n2)
      Node n1 = this->node1();
      Node n2 = this->node2();

      if(this->node2().index() < this->node1().index()){  //swap
        n1 = this->node2();
        n2 = this->node1();
      }

      for(size_type i = 0; i < graph->adj_list[n1.index()].size(); ++i){
        if(graph->adj_list[n1.index()][i].first == n2.index()){
          return graph->adj_list[n1.index()][i].second;
        }
      }
    }

    /** Return the const value of the Edge */
  ...
  bool has_edge(const Node& a, const Node& b) const {
    for (size_type i = 0; i < adj_list[a.index()].size(); i++) {
      if(adj_list[a.index()][i].first == b.index()){
        return true;
      }
    }
    return false;
  ...
  Edge add_edge(const Node& a, const Node& b, const edge_value_type& val = edge_value_type()) {
    ...
    adj_list[a.index()].push_back(std::make_pair(b.index(), val)); // (b, val)
    adj_list[b.index()].push_back(std::make_pair(a.index(), val)); // (a, val)
  ...
  /** A function that removes an edge from the graph.
   * @param n1    first node in edge to be removed
   * @param n2    second node in edge to be removed
   * @return      returns size_type value:
   *                1 if edge (@a n1, @a n2) was successfully removed
   *                0 if edge (@a n1, @a n2) does not exist in graph
   *
   * @pre         valid Graph::Node objects are passed in.
   * @post        edge (@a n1, @a n2) is removed from the graph. (Both elements
   *              (@a n1, @a n2) and (@a n2, @a n1) are removed from adj_list).
   * @post        numEdges decremented by 1.
   * @post        degree of each node, @a n1 and @a n2 is decremented by 1.
   *
   * Complexity: O(max_degree) (< O(num_nodes + num_edges))
   */
  size_type remove_edge(const Node& n1, const Node& n2){
    ...
    for(size_type i = 0; i < adj_list[n1.index()].size(); i++){
      if(adj_list[n1.index()][i].first == n2.index()){
        adj_list[n1.index()].erase(adj_list[n1.index()].begin()+i);
        break;
      }
    }
    for(size_type k = 0; k < adj_list[n2.index()].size(); k++){
      if(adj_list[n2.index()][k].first == n1.index()){
        adj_list[n2.index()].erase(adj_list[n2.index()].begin()+k);
        break;
      }
    }
    --numEdges;
  ...
  /* An edge e is deleted from the graph.
   * @param   e    a valid edge be deleted from the graph.
   * @return      returns size_type value:
   *                1 if edge (@a n1, @a n2) was successfully removed
   *                0 if edge (@a n1, @a n2) does not exist in graph
   *
   * This function calls size_type remove_edge(const Node& n1, const Node& n2). Thus, the @pre
   * & @post conditions and the complexity are the same as those listed for
   * size_type remove_edge(const Node& n1, const Node& n2).
   *
   */
  ...
  /* An edge pointed at by a valid edge iterator is deleted from the graph.
   * @param e_it  a valid edge iterator pointing to a valid edge to be deleted from the graph.
   * @return      returns an updated edge_iterator pointing to the next edge
   *              after the edge to be deleted.
   *
   * This function calls size_type remove_edge(const Node& n1, const Node& n2). Thus, the @pre
   * & @post conditions and the complexity are the same as those listed for
   * size_type remove_edge(const Node& n1, const Node& n2).
  ...
  class IncidentIterator : private totally_ordered<IncidentIterator> {
   public:
     ...
     */
    Edge operator*() const{
      return Edge(currGraph, n1, currGraph->adj_list[n1][n2].first);
    }
  ...
  class EdgeIterator : private totally_ordered<EdgeIterator>{
   public:
     ...
     */
    Edge operator*() const{
      return Edge(currGraph, currNode, currGraph->adj_list[currNode][currNeigh].first);
    }
    ...
    EdgeIterator& operator++(){
      ...
      while(currNode < currGraph->adj_list.size() && (currNode > currGraph->adj_list[currNode][currNeigh].first)){
 ...
 private:
  ...
  std::vector<std::vector<std::pair<size_type, edge_value_type>>> adj_list; // adjacency list that stores edges
