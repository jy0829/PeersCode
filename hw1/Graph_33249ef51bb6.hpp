...
class Graph{
 ...
 public:
  ...
  using size_type = unsigned;
	...
	inline size_type min(size_type x, size_type y) {
		return x < y ? x : y; 
	}
	inline size_type max(size_type x, size_type y) { 
		return x > y ? x : y; 
	}
  ...
  class Node : private totally_ordered<Node> {
   public:
    ...
    // incident_iterator edge_end() const;
		...
		incident_iterator edge_end() const{
			return IncidentIterator(graph_, uid_, graph_->edges_[uid_].size());
		}
  ...
  bool has_node(const Node& n) const {
    // HW0: YOUR CODE HERE
    return n.graph_ == this and n.uid_ < nodes_.size();
  }
   ...
   public:
    ...
    bool operator==(const Edge& e) const {
      return graph_==e.graph_ 
				...
				and max(uid1_, uid2_)==max(e.uid2_, e.uid2_);
    }
   ...
   public:
    ...
    // bool operator==(const NodeIterator&) const
		...
		/** Test equality for two node iterators
			* Two node iterators are equal if they have same @a graph_ and same @a i_
			*/
		bool operator==(const node_iterator& nit) const{
			return graph_==nit.graph_ and i_==nit.i_;
		}

   private:
    friend class Graph;
    // HW1 #2: YOUR CODE HERE
		NodeIterator(graph_type *graph_, size_type i_) :
			graph_(graph_), i_(i_) {}
  ...
  // node_iterator node_end() const
	...
	node_iterator node_end() const{
		return NodeIterator(const_cast<graph_type*>(this), nodes_.size());
	}
   ...
   public:
    ...
    // bool operator==(const IncidentIterator&) const
		...
		/** Returns the edge to which the iterator is referrring to.
			...
			* @pre 0 <= i_ < @a edges_[uid].size()
			*/
		Edge operator*() const{
	  	return Edge(graph_, uid_, graph_->edges_[uid_][i_]);
		}
		...
		/** Test two iterators for equality
			...
			* @a graph_, @a, @a uid_ and @a i_
			*/
		bool operator==(const incident_iterator& iit) const{
			return graph_==iit.graph_ and uid_==iit.uid_ and i_==iit.i_;
		}

   private:
    friend class Graph;
    // HW1 #3: YOUR CODE HERE
		IncidentIterator(graph_type* graph_, size_type uid_, size_type i_)
		 	:	graph_(graph_), uid_(uid_), i_(i_) {}
  ...
  class EdgeIterator : private equality_comparable<EdgeIterator> {
   public:
    ...
    // bool operator==(const EdgeIterator&) const
		...
		/** Returns the edge referred to by the edge iterator
			* @pre 0 <= uid_ < num_nodes()
			* @pre 0 <= i_ < @a graph_->edges_[uid_].size()
			*/
		Edge operator*() const{
			return Edge(graph_, uid_,
					graph_->edges_[uid_][i_]);
		}
		...
		/** Increment the edge iterator to refer to the next edge
			...
			*/
		EdgeIterator& operator++(){
			if(++i_==graph_->edges_[uid_].size()){
				i_=0;
				++uid_;
			}
			if(uid_==graph_->edges_.size() or uid_<graph_->edges_[uid_][i_])
				return *this;
			else
				return ++*this;
		}
		...
		/** Tests two iterators for equality
			...
			* @a graph_, @a uid_ and @a i_
			*/
		bool operator==(const EdgeIterator& eit) const{
			return graph_==eit.graph_ and uid_==eit.uid_ and i_==eit.i_;
		}
   ...
   private:
    friend class Graph;
    // HW1 #5: YOUR CODE HERE
		EdgeIterator(graph_type* graph_, size_type uid_, size_type i_) :
			graph_(graph_), uid_(uid_), i_(i_) {}
  ...
  edge_iterator edge_begin() const{
		return EdgeIterator(const_cast<graph_type*>(this), 0, 0);
	}
